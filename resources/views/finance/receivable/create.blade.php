<x-layout>
    ...
    <div id="successModal" class="fixed inset-0 bg-gray-900 bg-opacity-70 flex items-center justify-center z-50">
        <div class="bg-white rounded-2xl p-8 w-full max-w-sm text-center shadow-2xl border-4 border-emerald-600">
            <h3 class="text-2xl font-bold text-gray-900 mb-2">Data Tersimpan!</h3>
            <div class="text-6xl text-emerald-600 mb-4"><i class="fas fa-check-circle"></i></div>
    ...
    <div class="flex justify-between items-center mb-2">
        <h2 class="text-xl font-bold text-gray-700"><i class="fas fa-desktop mr-2"></i>Entry Pelunasan Piutang</h2>
    ...
    <div class="bg-gray-300 p-1 rounded border border-gray-500 shadow-xl">
        <div class="bg-gradient-to-b from-gray-200 to-gray-400 border border-gray-400 p-1 rounded-t mb-1">
            <span class="text-xs font-bold text-gray-700 px-2">ENTRY PELUNASAN PIUTANG</span>
        </div>

        <form action="{{ route('finance.receivable.store') }}" method="POST" class="bg-gray-200 p-3 border border-gray-400 rounded">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-2 mb-4">
                <div class="space-y-1">
                    <div class="flex items-center">
                        ...
                        <input type="text" name="payment_number" value="{{ $code }}" class="flex-1 text-xs font-bold border border-gray-400 bg-emerald-50 px-2 py-1 shadow-inner" readonly>
                    </div>
                    <div class="flex items-center">
                    ...
                    <div class="flex items-center">
                        <label class="w-32 text-xs font-bold text-gray-800">KODE KOLEKTOR</label>
                        <select name="collector_id" class="flex-1 text-xs border border-gray-400 px-2 py-1 shadow-inner bg-white">
                            <option value="">-- Pilih Kolektor --</option>
                            @foreach($users as $u) <option value="{{ $u->id }}">{{ $u->name }}</option> @endforeach
                        </select>
                    </div>
                    <div class="flex items-center">
                        <label class="w-32 text-xs font-bold text-gray-800">NAMA SALES</label>
                        <select name="sales_id" class="flex-1 text-xs border border-gray-400 px-2 py-1 shadow-inner bg-white">
                ...
                <div class="space-y-1">
                    <div class="flex items-center">
                        <label class="w-32 text-xs font-bold text-gray-800">KODE PELANGGAN</label>
                        <select name="customer_id" id="customerSelect" class="flex-1 text-xs border border-gray-400 px-2 py-1 shadow-inner bg-white" onchange="resetTable()" required>
                            <option value="">-- Pilih Customer --</option>
                            @foreach($customers as $cust) <option value="{{ $cust->id }}">{{ $cust->name }}</option> @endforeach
                        </select>
                    </div>
                    <div class="flex items-center">
                ...
                <table class="w-full text-xs border-collapse">
                    <thead class="bg-gray-100 border-b border-gray-400">
                        <tr>
                            ...
                            <th class="p-2 border-r text-right w-1/6">Sisa Piutang</th>
                    ...
                    <tfoot class="bg-gray-200 font-bold text-gray-700">
                        <tr>
                            <td colspan="3" class="p-2 text-right">TOTAL TERIMA :</td>
            ...
            <div class="flex flex-col md:flex-row gap-4 items-end justify-between">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded text-sm font-bold shadow-lg flex items-center">
    ...
    <div id="invoiceModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white w-full max-w-4xl rounded shadow-xl border border-gray-500 overflow-hidden flex flex-col h-[500px]">
            <div class="bg-emerald-700 px-4 py-2 flex justify-between items-center text-white">
                <h3 class="font-bold text-sm">PILIH FAKTUR PIUTANG</h3>
                ...
                <table class="w-full text-xs text-left border-collapse">
                    <thead class="bg-gray-200 border-b border-gray-400 sticky top-0">
                        <tr>
                            ...
                            <th class="p-2 border-r text-right">Sudah Diterima</th>
                            <th class="p-2 border-r text-right">Retur</th>
                            <th class="p-2 border-r text-right">Sisa Piutang</th>
    ...
    <script>
        ...
        function addRow() {
            ...
            tr.innerHTML = `
                ...
                <td class="p-1 border-r">
                    <input type="number" name="details[999][amount]" class="amount-input w-full text-right font-bold border border-gray-300 rounded px-1 py-1 focus:ring-2 focus:ring-emerald-400" required oninput="calculateTotal()">
                </td>
                <td class="p-1 border-r">
        ...
        function openModalForRow(element) {
            ...
            if(!custId) { alert("Pilih Customer dulu!"); return; }
            ...
            // PENTING: Route getUnpaidSales
            fetch(`/keuangan/piutang/get-sales/${custId}`)
                .then(res => res.json())
                .then(data => {
                    tbody.innerHTML = '';
                    if(data.length === 0) {
                        tbody.innerHTML = '<tr><td colspan="5" class="p-4 text-center text-gray-500">Tidak ada piutang.</td></tr>'; return;
                    }
                    data.forEach(inv => {
                        const row = `
                            <tr class="hover:bg-emerald-50 cursor-pointer" ondblclick='selectInvoice(${JSON.stringify(inv)})'>
                                <td class="p-2 border-r font-bold text-emerald-800">${inv.text}</td>
                                ...
                                <td class="p-2 text-center">
                                    <button type="button" onclick='selectInvoice(${JSON.stringify(inv)})' class="bg-emerald-600 text-white px-2 py-0.5 rounded text-xs">Pilih</button>
                                </td>
                            </tr>
                        `;
        ...
        function selectInvoice(inv) {
            ...
            tr.querySelector('.remaining-debt').value = fmt(inv.remaining); 
